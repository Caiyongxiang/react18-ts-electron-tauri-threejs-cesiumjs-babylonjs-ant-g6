...
const codes = [
  {
    p: `MeshMatcapMaterial是一种特殊的材质，它使用一种叫做"Matcap"的纹理（texture）来模拟光照和反射效果。它不依赖于场景中的实际光源，而是通过一个预先生成的纹理来决定模型的外观<br>Matcap（Material Capture）是一种纹理图像，通常是球形的，包含了特定光照条件下的材质效果。它捕捉了光源、阴影和反射的信息。你可以把它看作是一个带有特殊光照效果的“贴图`,
    language: 'javascript',
    value: `    gltfLoader.load(
      // 模型路径
      '../../../model/Duck.glb',
      // 加载完成回调
      gltf => {
  ...
  {
    p: `MeshLambertMaterial、MeshPhongMaterial 和 MeshStandardMaterial 的区别<br>
...
// 创建平面
  ...
  {
    p: `MeshPhysicalMaterial 是 Three.js 中一种用于创建具有物理属性的材质<br>
...
reflectivity: 0.99 使金属表面反射环境光，看起来非常真实。
  ...
  {
    p: `refractionRatio：你在设计一个简单的玻璃窗，使用 refractionRatio 可以快速实现光线的基本折射效果。<br>
ior：你在设计一个精致的水晶吊灯，需要精确模拟光线在水晶中的折射和反射效果，使用 ior 更加合适<br>
    `,
    language: 'javascript',
    value: `   gui.add(cube.material, 'ior', 0, 2).name('折射率')
  // reflectivity
...
举例：你在设计一款钢琴，钢琴表面有细小的划痕，你可以使用 clearcoatNormalMap: scratchNormal 来添加这些细小的划痕，使表面看起来更真实。<br>
normalMap
定义：使用法线贴图控制材质本身的细节。<br>
作用：模拟材质表面的凹凸和纹理，使表面更逼真。<br>
举例：你在设计一块碳纤维面板，面板表面有特有的纹理，你可以使用 normalMap: carbonNormal 来添加这些碳纤维纹理，使面板看起来更加逼真。<br>
...
举例：你在设计一个汽车模型，想让车漆上的细小划痕不那么明显，可以使用 clearcoatNormalScale: new THREE.Vector2(0.1, 0.1) 来减弱划痕的强度，使划痕看起来更加微小和不明显。
  ...
  clearcoatNormalMap: scratchNormal, // 使用划痕法线贴图
  normalMap: carbonNormal, // 使用碳纤维法线贴图
...
举例：你在设计一个砖墙，想要砖墙表面的光泽效果有不同颜色，可以使用 sheenColorMap: brickRoughness，使光泽颜色随贴图变化。
    `,
    language: 'javascript',
    value: `
  ...
  sheenColorMap: silkTexture // 使用丝绸纹理贴图
});`,
    height: '100px',
    theme: 'vs-dark'
  },
...
举例：你在设计一个虹彩效果的表面，想要不同部分显示不同的颜色，可以使用 iridescenceThicknessMap: brickRoughness。
    `,
    language: 'javascript',
    value: `
  ...
  iridescenceThicknessMap: brickRoughness // 使用贴图控制虹彩厚度
});
`,
    height: '100px',
    theme: 'vs-dark'
  ...
  },
  {
    ...
    `,
    language: 'javascript',
    value: `let controls = addControls()
    // 设置带阻尼的惯性
