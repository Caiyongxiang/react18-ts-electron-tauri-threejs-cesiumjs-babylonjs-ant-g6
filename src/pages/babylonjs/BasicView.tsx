...
const BasicView = () => {
  const code = `this.canvas = document.getElementsByClassName(
      "cyxScene"
    ...
    <div style={{ width: '100%', height: '100%' }}>
      ...
      <Drawer title="笔记内容" onClose={onClose} open={open} width={600} style={{ overflow: 'hidden', height: 'calc(100%)' }}>
        <div style={{ height: '100%', overflow: 'hidden' }}>
          ...
          <p>创建场景步骤：场景绑定引擎，创建相机绑定Demo元素，创建灯光和盒子绑定场景</p>
          <Editor options={options} height="280px" language="javascript" value={alcode} theme="vs-dark" />
        </div>
      </Drawer>
    </div>
  )
